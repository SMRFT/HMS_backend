...
Start-Process powershell -ArgumentList "-NoExit", "-Command", ".\env\Scripts\python.exe manage.py runserver 0.0.0.0:2609"

# Start Automation Worker in daemon mode in a new window using venv
Write-Host "Starting Automation Worker (Daemon)..."
Start-Process powershell -ArgumentList "-NoExit", "-Command", ".\env\Scripts\python.exe manage.py send_pending_payments_report --daemon"
